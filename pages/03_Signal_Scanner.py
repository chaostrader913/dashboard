...
with col2:
    # Defaulting to Daily chart ('1d') as requested
    timeframe = st.selectbox("TIMEFRAME", options=["1d", "1h", "15m"], index=0)
with col3:
    # Defaulting to 'mike' style as requested
...
# Ensure dataframe index is a valid DatetimeIndex for mplfinance
...
# --- 5. Charting Engine (mplfinance Dashboard) ---
st.markdown("### VISUALIZATION PANE")

# Interactive tabs provide a cleaner look as PnF and Renko redefine x-axis intervals
tab1, tab2, tab3 = st.tabs(["Point & Figure (PnF)", "Renko Chart", "Heikin Ashi Candlechart"])

with tab1:
    st.markdown(f"#### Point & Figure (PnF): {ticker}")
    try:
        fig1, axlist1 = mpf.plot(plot_df, type='pnf', style=theme_sel, returnfig=True)
        st.pyplot(fig1)
        plt.close(fig1)
    except Exception as e:
...
with tab2:
    ...
    try:
        fig2, axlist2 = mpf.plot(plot_df, type='renko', mav=(5, 10, 20), style=theme_sel, returnfig=True)
        st.pyplot(fig2)
        plt.close(fig2)
    except Exception as e:
...
with tab3:
    ...
    try:
        ...
        # 2. Jurik MA Line (Dynamically matching possible column names)
